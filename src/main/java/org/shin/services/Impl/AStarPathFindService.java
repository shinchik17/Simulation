package org.shin.services.Impl;

import org.shin.services.APathFindService;
...
public class AStarPathFindService extends APathFindService {
    ...
    /**
     * Находит кратчайший путь от заданного начального узла до целевого узлу с использованием алгоритма A*.
     *
     * @param start  начальная клетка
     * @param target целевая клетка
     * @param map    карта, содержащая клетки и препятствия
     * @return кратчайший путь в виде ArrayDeque клеток, или null, если путь не найден
     */
    public static ArrayDeque<Cell> findPath(Cell start, Cell target, Map map) {
        ...
        ArrayDeque<Cell> path = nodeList.stream()
                .map(n -> new Cell(n.x, n.y))
...
//        path.removeLast();
    ...
    /**
     * CODEIUM
     * Находит допустимые соседние узлы заданного узла на карте, исключая любые клетки, которые являются препятствиями.
     *
    ...
    private static List<Node> findValidNeighbours(Node node, Map map) {
        ...
        return neighbourCells.stream().filter(x -> !obstaclesMap.containsKey(x)).map(x -> new Node(x)).collect(Collectors.toList());
    }


    /**
     * Класс имплементирующий узел для использования в алгоритме A*
     */
